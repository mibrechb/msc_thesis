...
// Filter imagecollection for polarization to get imagecollection with only one mode
var s1_filt_vh = s1_filt
...
var s1_dateList = s1_filt_vh.aggregate_array('date')

// Output usable S-1 imagecollection and available dates
print('Usable S-1 Collection:', s1_filt_vh)
print('Available S-1 dates:', s1_dateList)
...
/*
// Function to find bufferpolygon around lake for soil training data
var getBufferPolygon = function(image){
  // Get image id
  var id = image.id()
  // Find date
  var im_date = ee.String(image.date().format('YYYY-MM-dd'))
  // Add training Polygon
  var polygon_buffered = ee.Feature(ground).set({id: id}).set({class: 3})
  return(polygon_buffered)
}
*/

// Filter available S-1 images with available coarse classification and find corresponding labels
var training = s1_filt_vh
  .filter(ee.Filter.inList('date', coarseClassification_dateList))
  .map(getClassLabel)

/*
// Add 50 random bufferpolygons as soil training data
var training_buffer = ee.FeatureCollection(s1_filt_vh
  .filter(ee.Filter.inList('date', coarseClassification_dateList))
  .map(getBufferPolygon)
  .randomColumn('random')
  .sort('random')
  .toList(200)
  .slice(0,50))
